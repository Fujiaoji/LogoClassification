...
class Brand277Dataset(Dataset):
    def __init__(self, csv_file, transform=None, norm=True, target_transform=None):
    ...
    def __getitem__(self, idx):
        img_path = self.img_df.iloc[idx, 0] # 0 means logo_path
...
if __name__ == '__main__':
    """Load the classifier"""
    # The model can classify the inpu logo to 277 classes
    ...
    transform = transforms.Compose([
        ...
        transforms.Resize((128, 128)),                # Resize to 128
        transforms.ToTensor(),                      # Convert to tensor
        transforms.Normalize(mean=[0.5, 0.5, 0.5],  # Normalize
                             std=[0.5, 0.5, 0.5])
    ])
    with open('dataset/brand277_target.json') as f:
        target_dict = json.load(f)
    
    # csv file that contains the image path, brand
    test_data = Brand277Dataset(csv_file="dataset/brand277_test.csv",
                             transform=transform,
                             target_transform=target_dict)
    test_dataloader = DataLoader(test_data, batch_size=2, shuffle=False) # batchsize
    for idx, data in enumerate(test_dataloader): # torch.Size([N, 3, 128, 128]), data[0]: image, data[1]: label
        logits = model(data[0]) # input image to get the pred label
        v_possibility = F.softmax(logits, dim=-1) # softmax
        v_pred = torch.argmax(v_possibility, dim=-1) # predict label
        max_possibility = torch.gather(v_possibility, dim=-1, index=v_pred.unsqueeze(-1)).squeeze(-1)
        print(v_pred, data[1], max_possibility) # this is the pred brand, true brand, and the pred brand possibilities
        break